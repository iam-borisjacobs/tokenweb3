    ...
    public function authenticate(Request $request, $social)
    {
        ...
        if ($user) {
            ...
            if (session('ref_by')) {
                ...
                $ref_user = User::where('username', $ref_by)->first();
                $ref_by_id = $ref_user ? $ref_user->id : NULL;
            } else {
                $ref_by_id = NULL;
            }
            ...
            try {
                ...
                $objDemo->sender = $settings->site_name ?? config('app.name');
                $objDemo->contact_email = $settings->contact_email ?? config('mail.from.address');
                Mail::to($userSocial->getEmail())->send(new NewRegistration($objDemo));
            } catch (\Throwable $e) {
                \Illuminate\Support\Facades\Log::error('Error sending social registration email: ' . $e->getMessage());
            }
