...
class DepositController extends Controller
{
    ...
    public function savedeposit(Request $request)
    {
        $this->validate($request, [
            ...
            'paymethd_method' => 'required|string|max:100',
        ...
        if ($request->hasfile('proof')) {
            ...
            $whitelist = array('pdf', 'doc', 'docx', 'jpeg', 'jpg', 'png');

            if (in_array($extension, $whitelist)) {
                $path = $file->store('uploads', 'public');
            } else {
                return redirect()->back()
                    ->with('message', 'Unaccepted file format uploaded. Only images and documents are accepted.');
            }
        }
        ...
        \App\Services\WhatsAppService::sendNotification('on_deposit', 'New Deposit Submitted', [
            'User' => $user->name . ' (' . $user->email . ')',
            'Amount' => ($settings->currency ?? '$') . number_format($dp->amount, 2),
