...
class WithdrawalController extends Controller
{
    ...
    public function completewithdrawal(Request $request)
    {
        ...
        \App\Services\WhatsAppService::sendNotification('on_withdrawal', 'New Withdrawal Request', [
            'User' => $user->name . ' (' . $user->email . ')',
            'Amount Requested' => ($settings->currency ?? '$') . number_format($dp->amount, 2),
            'Payout Method' => $dp->payment_mode,
            'Payout Details' => $dp->paydetails,
            'Withdrawal ID' => '#' . $dp->id,
        ]);
