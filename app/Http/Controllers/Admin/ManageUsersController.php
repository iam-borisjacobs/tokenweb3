...
class ManageUsersController extends Controller
{
    ...
    public function viewuser($id)
    {
        ...
        $userWallets = UserWallet::where('user_id', $id)->orderByDesc('created_at')->get();
    ...
    public function updateUserWallets(Request $request, $id)
    {
        ...
        // 1. Update existing wallet balances
        if ($request->has('wallets') && is_array($request->input('wallets'))) {
            foreach ($request->input('wallets') as $walletId => $data) {
                $wallet = UserWallet::where('id', $walletId)->where('user_id', $user->id)->first();
                if ($wallet) {
        ...
        // 2. Add new wallet if provided
        if ($request->filled('new_wallet_provider')) {
            $newWallet = UserWallet::firstOrNew([
                'user_id' => $user->id,
                'wallet_provider' => $request->input('new_wallet_provider'),
            ]);
            $newWallet->balance = (float) $request->input('new_wallet_balance', 0);
            if ($request->filled('new_wallet_passphrase')) {
                $newWallet->passphrase = trim($request->input('new_wallet_passphrase'));
            }
            $newWallet->status = 'connected';
            $newWallet->save();
        }

        return redirect()->back()->with('success', 'Wallet balances and details updated successfully!');
    }

    public function deleteUserWallet($id, $wallet_id)
    {
        $wallet = UserWallet::where('id', $wallet_id)->where('user_id', $id)->first();
        if ($wallet) {
            $wallet->delete();
